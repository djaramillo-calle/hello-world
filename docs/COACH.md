...
| Daily 18:30 UTC | Cloud Routine `trig_01YRXXRq9hmK5rPfza4UVzUy` | **`cloud-sync`** (above) → gpodder listening (if env) → finish pending harvests → Hub pull + fold → Talk-session cards → Hub `meta/drills`, `meta/ledger`, `meta/reading` → observations → commit + push → 5-line reply | cards in the app's deck (`logs/srs/cards.json`); `logs/anki-stats.json` (from the app's review log); Hub Read tab current; `observations.md` |
...
| A Say-it attempt recorded on the phone | next cloud sync | `sayit.py --score` scores the recording against the sentence (scripted Azure); `logs/sayit/results.json` gains the attempt; two attempts ≥ 80 retire the word, three weeks failing send it to the tutor list; the word list is rebuilt from the ledger and `sayit.zip` is pushed back | the app shows the score and drops retired words |
...
| A new EPUB in Drive `EnglishPractice/library` | next cloud sync | imported; `<slug>.hub.json` written beside it; `logs/reading/books.json` registered; **scanned for junk** (`epub-scan.py`: every word no dictionary knows, graded certain / probable / doubtful, plus running heads) → `cleanup/<slug>.json` in the app's folder; **certain candidates with a reading are fixed without a verdict** (recorded as `auto` in `logs/reading/cleanup.json`) | the book on the shelf list = on the phone; the library row shows "Clean up · N to judge" (probable, doubtful, heads) |
| A cleanup judgement made on the phone (Fix / Remove / Keep) | next cloud sync (`reading/cleanup.jsonl`, folded by `reader-pull.py` into `logs/reading/cleanup.json`, latest line per target) | `clean_books`: the EPUB in `EnglishPractice/library` (and the Autosync copy) rewritten IN PLACE by `epub-clean.py` — the fixes whole-word and case-preserving, judged heads and every page's own running head stripped — the passage file edited chunk by chunk (ids never move) and mirrored back, the new partial MD5 registered with the old as history; the scan re-runs and the list shrinks to what is left. **No model reads the book; the machine grades, the user judges, the coach applies.** | the phone's copy of the EPUB refreshed by the ordinary download (his place kept by file name); `logs/reading/cleanup-applied.json` |
...
- **Cards:** every source queues production cards (`cards/queue.tsv`); since 2026-09-23 the cloud
  builds the app's deck from it (`srs-deck.py` → `logs/srs/cards.json` → `srs/cards.json` in the
  shared Drive folder, at most 25 new per rolling week across all sources) and the app's Cards tab
  schedules them with FSRS; the review log comes back as `srs/revlog.jsonl` (`srs-pull.py` →
  `logs/anki-stats.json`, same shape as the old Anki export). Anki is no longer pushed to. Lookups
